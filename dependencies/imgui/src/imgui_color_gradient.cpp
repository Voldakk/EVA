...
namespace ImGui
{
    static void DrawGradientBar(ImGradient* gradient, struct ImVec2 const& bar_pos, float maxWidth, float height)
    {
        ...
        {
            draw_list->AddRectFilledMultiColor(ImVec2(prevX, bar_pos.y), ImVec2(bar_pos.x + maxWidth, barBottom), colorBU32, colorBU32,
                                               colorBU32, colorBU32);
        }
    ...
    static void DrawGradientMarks(ImGradient* gradient, ImGradientMark*& draggingMark, ImGradientMark*& selectedMark,
    ...
    bool GradientEditor(ImGradient* gradient, ImGradientMark*& draggingMark, ImGradientMark*& selectedMark)
    {
        ...
        if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0))
        {
            ...
            auto it = std::find_if(gradient->getMarks().begin(), gradient->getMarks().end(), [&pos](const ImGradientMark* m) { return m->position == pos; });
            if (it != gradient->getMarks().end()) selectedMark = *it;
        }
        ...
        if (selectedMark && ImGui::IsWindowFocused() && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Delete))) 
        {
        ...
        if (!selectedMark && gradient->getMarks().size() > 0) { selectedMark = gradient->getMarks().front(); }
