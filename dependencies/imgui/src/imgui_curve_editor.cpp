...
namespace ImGui
{
    ...
    int CurveEditor(const char* label, float* values, int points_count, const ImVec2& editor_size, ImU32 flags, int* new_count)
    {
        ...
        {
            ImVec2 point;
            if (flags & (int)CurveEditorFlags::NO_TANGENTS) { point = ((ImVec2*)values)[point_idx]; }
            else
            {
        ...
        if (flags & (int)CurveEditorFlags::SHOW_GRID)
        {
            ...
            for (int i = -1; i < cell_cols + 2; ++i)
            {
                ...
                char buf[64];
                if (exp > 0) { ImFormatString(buf, sizeof(buf), " %d", int(x + i * step_x)); }
                else
                {
            ...
            for (int i = -1; i < cell_rows + 2; ++i)
            {
                ...
                window->DrawList->AddLine(a, b, 0x55000000);
                char buf[64];
                if (exp > 0) { ImFormatString(buf, sizeof(buf), " %d", int(y + i * step_y)); }
                else
                {
        ...
        if (ImGui::IsMouseReleased(1)) { window->StateStorage.SetBool((ImGuiID)StorageValues::IS_PANNING, false); }
        ...
        {
            ImVec2* points;
            if (flags & (int)CurveEditorFlags::NO_TANGENTS) { points = ((ImVec2*)values) + point_idx; }
            else
            {
            ...
            ImVec2 tangent;
            ImVec2 p;
            if (flags & (int)CurveEditorFlags::NO_TANGENTS) { p = points[1]; }
            else
            {
            ...
            {
                window->DrawList->AddBezierCurve(transform(p_prev), transform(p_prev + tangent_last), transform(p + tangent), transform(p),
                ...
                if (handlePoint(p, 1))
                {
                    ...
                    if (point_idx < points_count - 2 && p.x >= points[6].x) { p.x = points[6].x - 0.001f; }
                    points[3]   = p;
                    changed_idx = point_idx + 1;
                }
            }
            else
            {
                ...
                {
                    if (p.x <= p_prev.x) p.x = p_prev.x + 0.001f;
                    if (point_idx < points_count - 2 && p.x >= points[2].x) { p.x = points[2].x - 0.001f; }
                    points[1]   = p;
                    changed_idx = point_idx + 1;
                }
            }
