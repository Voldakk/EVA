...
namespace nlohmann
{
    ...
    {
        static void to_json(json& j, const std::filesystem::path& path) { j = EVA::FileSystem::ToString(path); }

        static void from_json(const json& j, std::filesystem::path& path) { path = j.get<std::string>(); }
    };
} // namespace nlohmann
