...
layout(std430, binding = 2) restrict buffer indexBuffer { uint u_NextIndex; };

uniform float u_Threshold;

vec2 uv(int x, int y, ivec2 dims) { return vec2(x, y) / vec2(dims); }
...
void main()
{
    ...
    for (int localY = 0; localY < pixelsPerThread; localY++)
    {
        for (int localX = 0; localX < pixelsPerThread; localX++)
        {
            ...
            if (value > u_Threshold)
            {
                ...
                if (labelX == maxLabel && labelY == maxLabel) { localLabels[localX + localY * pixelsPerThread] = nextLabel++; }
                else
                {
    ...
    {
        uint l = links[i];
        if (l == i) { links[i] = atomicAdd(u_NextIndex, 1); }
        else
        {
            links[i] = links[l];
        }
