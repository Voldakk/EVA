...
float rand(vec2 n) 
{ 
	return fract(sin(dot(vec3(n, u_Seed), vec3(12.9898, 4.1414, 3.1415))) * 43758.5453);
}
...
float tile(vec2 uv, ivec2 index)
{
    return square(uv, index);
}
